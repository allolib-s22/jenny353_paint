...
Description:
Interact with paintbrush using ray intersection tests, and draw 3D spheres that
are mapped sonically to 2D coordinates
...
struct RayBrush : public DistributedAppWithState<SharedState> {
  ...
  std::vector<Color>
      colorSpheres; // keeps track of color for each drawing stroke
  std::vector<int>
      start_stroke_positions; // keeps track of where all the stroke positions
                              // start for the undo function
  ...
  void onInit() override {
    auto cbDomain = CuttleboneDomain<SharedState>::enableCuttlebone(this);
    synthManager.synthSequencer().registerSynth(scene);
    registerDynamicScene(scene);
    scene.registerSynthClass<Sound>();
  ...
  void onAnimate(double dt) override {
    ...
    // navControl().useMouse(!isImguiUsingInput()); //allows screen to move with
    // mouse navControl().active(!isImguiUsingInput());  // not sure what this
    // does

    // draw to shared state
    if (isPrimary()) {
      state().count = pos.size();
      ...
      pos.resize(count);
      colorSpheres.resize(count);
      for (int i = 0; i < count; i++) {
        pos[i] = state().position[i];
        colorSpheres[i] = state().color[i];
      }
    }
    if (hasCapability(CAP_2DGUI)) {
      imguiBeginFrame();
      ...
      if (move_with_mouse) {
        navControl().useMouse(
            !isImguiUsingInput()); // allows screen to move with mouse
      }
      // 3 settings Loop pedal: record, play, off
      // ImGui::RadioButton("Record Loop", recordLoop); //Spacebar is pressed
      // and loop recording is turned on ImGui::RadioButton("Play Loop",
      // playLoop);
      // //Spacebar is pressed while recordLoop is true, recordLoop becomes
      ...
      if (ImGui::Button("Undo")) {
        ...
        colorSpheres.erase(colorSpheres.begin() + getPrevStroke,
                           colorSpheres.end());
      }
      ImGui::End();
      imguiEndFrame();
  ...
  virtual void onDraw(Graphics &g) override {
    ...
    for (int i = 0; i < pos.size(); i++) {
      // std::cout<<"in onDraw, i = " << i << " pos: "<< pos[i] << "color = "
      // <<colorSpheres[i].y<<std::endl;
      g.pushMatrix();
      g.translate(pos[i]);
      // g.color(1, .87, .5);
    ...
    nav().pos(0, 0,
              synthManager.voice()->getInternalParameterValue(
                  "z")); // zoom in and out, higher z is out farther away
    light.pos(0, 0,
              synthManager.voice()->getInternalParameterValue(
                  "z")); // where the light is set

    if (!synthManager.synthSequencer().playing() &&
        playLoop == true) { // keeps looping if playLoop is on
      std::string sequenceName =
          "sound" + std::to_string(sequenceFileNum - 1); // loop prev file
      std::cout << "play sequence: " << sequenceName << std::endl;
      synthManager.synthSequencer().playSequence(sequenceName +
                                                 ".synthSequence");
    }
  }
  ...
  bool onMouseDown(const Mouse &m) override {
    ...
    if (isPrimary()) {
      ...
      start_stroke_positions.push_back(
          pos.size()); // set the last stroke positon to start from here
      ...
      // note mapping: lower left is lower freq, higher and up to right gives
      // higher freq
  ...
  bool onKeyDown(const Keyboard &k) override {
    ...
    if (k.key() == ' ' && recordLoop == false) { // start recording loop
      ...
      std::cout << "start recording in file: sequenceName " << sequenceName
                << std::endl;

      synthManager.synthRecorder().startRecord(sequenceName, true);

    } else if (k.key() == ' ' &&
               recordLoop == true) { // start playing recorded loop on repeat
                                     // with sequencer
